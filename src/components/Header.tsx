...
import { ServicesDropdownItem, ServicesAccordion } from "./ServicesDropdown";
import {
  NavigationMenu,
...
const Header = () => {
  ...
  // Mount reset and BFcache handling
  useEffect(() => {
    setMobileMenuOpen(false);
  ...
  return (
    ...
    >
      <motion.nav
        ...
        className="grid grid-cols-[auto_1fr_auto] items-center px-4 sm:px-6 lg:px-10 py-2 mb-5 sm:py-3 lg:py-4 rounded-full shadow-[0_1.25rem_3.125rem_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.05)]"
        role="navigation"
        aria-label="Main navigation"
      >
        {/* Logo Area */}
        <div className="flex-shrink-0 z-20 min-w-[140px] lg:min-w-[200px]">
        ...
        {/* Desktop Navigation - Centered in the middle but justify-start for trigger consistency */}
        {/* Desktop Navigation - Centrally aligned */}
        <NavigationMenu className="hidden lg:flex items-center justify-center max-w-none px-4">
          <NavigationMenuList className="gap-2 xl:gap-4">
            <ServicesDropdownItem dictionary={dictionary} locale={locale} />
            {navLinks.map((link) => (
              <NavigationMenuItem key={link.label}>
                <NavigationMenuLink asChild>
                  <Link
                    ...
                    className={cn(
                      navigationMenuTriggerStyle(),
                      "text-muted-foreground hover:text-primary transition-all text-base lg:text-lg font-bold tracking-wide whitespace-nowrap bg-transparent"
                    )}
        ...
        <div className="flex items-center justify-end gap-3 sm:gap-6 relative z-20 min-w-[140px] lg:min-w-[220px]">
